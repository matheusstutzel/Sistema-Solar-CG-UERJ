...
Asteroids::Asteroids(float dist_min, float dist_max, float tam_min, float tam_max, int count, double incl) {
	...
	for (int i = 0; i < count; i++) {
		...
		while (!inseriu && j<1000)//tenta inserir em um lugar sem colsão( até 1000 tentativas)
		{
			...
			temp->reset(dist, trans, tam, incl);//reaproveita objeto caso o anterior tenha colidido
			if (testa(*temp)) {
				a.push_back(*temp);
				inseriu = true;
			}
