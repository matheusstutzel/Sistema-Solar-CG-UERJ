...
float dist_min = 2.2 * 149600000;//2.2 dist Terra
float dist_max = 3.2 * 149600000;//3.2 dist Terra

float tam_min = 0.2 * 12756;//0.2 tam Terra
float tam_max = 0.4 * 12756;//0.4 tam Terra
...
void TecladoUP(unsigned char key, int x, int y) {
	//ao soltar a tecla o movimento deve parar
...
void idleFunc(void) {
	...
	for (int i = 0; i < maxFig; i++)for (int j = 0; j < 3; j++) {
		...
		pos[i][j] += inc[i][j];
		//você não pode sair do universo
		if (i == 0)pos[i][j] = max(pos[i][j], -sizeOfUniverse / 10);
		if (i == 0)pos[i][j] = min(pos[i][j], sizeOfUniverse/10);
		//enquanto a tecla estiver pressionada o moviento deve aumentar 
		inc[i][j] *= 1.02;
	}
	glutPostRedisplay();
}
...
void main(int argc, char **argv)
{
	...
	glutCreateWindow("Matheus Stutzel Solar System");
...
GLuint loadTexture(char *filename) {
	...
	glTexImage2D(GL_TEXTURE_2D, //Always GL_TEXTURE_2D
		...
		GL_UNSIGNED_BYTE, //GL_UNSIGNED_BYTE:pixels are stored as unsigned numbers
...
void desenhaFundo() {
	...
	//desenhar a textura do lado de dentro 
	...
	//esfera q representa o universo ao fundo
	gluSphere(quad, sizeOfUniverse, 500, 500);
	
	//desenhar a textura do lado de fora 
